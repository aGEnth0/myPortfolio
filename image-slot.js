...
(() => {
  ...
  // ── Sharded persistence ────────────────────────────────────────────────
  // The host refuses any single writeFile past ~2MB, so one monolithic
  // sidecar can't grow with a large gallery — new drops would silently fail
  // to save. Photos are split across BUCKETS small files (each well under the
  // cap), keyed by a stable hash of the slot id. The pre-shard single file is
  // still read so older sidecars migrate transparently.
  const BUCKETS = 16;
  const fileFor = (b) => '.imgslots-' + b + '.state.json';
  ...
  function mergeInto(j) {
    // Merge: on-disk loses to any in-memory change that raced ahead of the
    ...
    // user image that's only on disk — inherit u from disk where missing.
  ...
  function load() {
    ...
    const files = [];
    for (let b = 0; b < BUCKETS; b++) files.push(fileFor(b));
    files.push(STATE_FILE); // backward-compat: absorb pre-shard sidecar
    loadP = Promise.all(files.map((f) =>
      fetch(f).then((r) => (r.ok ? r.json() : null)).catch(() => null)
    ))
      .then((parts) => { for (const j of parts) mergeInto(j); tombstones.clear(); })
      .catch(() => {})
      .then(() => { loaded = true; subs.forEach((fn) => fn()); });
    return loadP;
  }

  // Per-bucket serialized writes. Only buckets whose contents changed since
  // the last successful write are re-sent, so a single drop touches one small
  // file. A save requested mid-flight marks dirty and re-fires on completion.
  const lastWritten = {};
  let saving = false;
  let saveDirty = false;
  function save() {
    ...
    if (!w) return;
    const grouped = {};
    for (let b = 0; b < BUCKETS; b++) grouped[b] = {};
    for (const id in slots) grouped[bucketOf(id)][id] = slots[id];
    const writes = [];
    for (let b = 0; b < BUCKETS; b++) {
      const json = JSON.stringify(grouped[b]);
      if (json !== (lastWritten[b] || '{}')) writes.push([b, json]);
    }
    if (!writes.length) return;
    saving = true;
    Promise.all(writes.map(([b, json]) =>
      Promise.resolve(w(fileFor(b), json)).then(() => { lastWritten[b] = json; }).catch(() => {})
    ))
